...
export class CascataScene extends Phaser.Scene {
  ...
  public setupLevel(levelNumber: number): void {
    ...
    this.metas = config.targets.map((target) => ({
  ...
  private handleRobotDrop(operation: TipoDeOperacaoDaCascata, slotId: string): void {
    ...
    this.metas = this.metas.map((meta) =>
      meta.output === slotView.output
        ? { ...meta, current: this.applyOperation(meta.current, operation) }
        : meta
    );
  ...
  private checkWin(): void {
    const venceu = this.metas.every((meta) => meta.current === meta.goal);
    const tudoRestaurado = this.metas.every((meta) => this.restoredTopics.has(meta.output));
    if (!venceu || !tudoRestaurado) {
      return;
    }
    ...
    this.resultText?.setText(
      this.currentSubLevel === 1
        ? "Subnivel 1 concluido. Clique em Next para ir para a Fase 02."
        : "Cascata concluida. Clique em Next para voltar ao mapa."
    );
    this.playRestoreTransition();
  }

  private renderMetaValues(): void {
    ...
    const healthPercent = this.metas.length > 0 ? Math.round((desafiosConcluidos / this.metas.length) * 100) : 0;
    this.redrawEcosystemHealthBar(healthPercent);
  }

  private calcularProgresso(meta: MetaDaCascata): number {
  ...
  private getCurrentEcosystemHealth(): number {
    ...
    const concluidos = this.metas.filter((meta) => meta.current === meta.goal).length;
